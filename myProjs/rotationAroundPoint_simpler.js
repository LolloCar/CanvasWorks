...
const sketch = ({ context, width, height }) => {
	...
	return ({ context, width, height }) => {
		...
		circle(context,a.x,a.y,10);
		//Main animation
		let rotation =t/20;
		context.translate(p.x,p.y);
		context.rotate(rotation);
		context.strokeStyle = 'blue';
		//notice that you have to compensate for the position of points
		context.strokeRect(a.x-p.x,a.y-p.y,boxSide,boxSide);
		context.beginPath();
		context.moveTo(a.x-p.x,a.y-p.y);
		context.lineTo(a.x-p.x+boxSide,a.y-p.y+boxSide);
		context.stroke();
		t++;
